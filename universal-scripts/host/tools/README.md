...
- Insert the prepared SD card if ULoad or rootfs flashing is selected.
...
- After selecting the boot mode, restart the board with RESET or a power-cycle
  as documented for each board below.
...
flowchart TD
  ...
  T --> U1[RESET or power-cycle to normal boot mode]:::action
  S -->|No| U1
