...
class BootloaderFlashUtil:
	...
	def __validate_v4h_xspi_layout(self, board_config):
		"""Reject V4H SPI payloads that would overlap a following region."""
		if not self.__is_v4h() or self.__args.flashMethod != "xspi":
			return

		try:
			flash_address = board_config["xspi"]
			xspi_region_start = int(board_config["xspi_region_start"], 16)
			xspi_region_size = int(board_config["xspi_region_size"], 16)
			if xspi_region_size <= 0:
				raise ValueError("xspi_region_size must be greater than zero")
		...
		for index, (label, path, start) in enumerate(spans):
			...
			if start < xspi_region_start or end > xspi_region_end:
				die(msg=(f"V4H xSPI overflow: {label} [0x{start:X}, 0x{end:X}) "
						 f"is outside the configured xSPI region "
						 f"[0x{xspi_region_start:X}, 0x{xspi_region_end:X})"))
	...
	def writeBootloader(self):
		...
		if not os.path.exists(self.__args.bidImage):
			...
			exit()
		self.__validate_v4h_xspi_layout(self.__flashAddress)

		# Wait for device to be ready to receive image.
		if self.__is_v4h():
			print("\nSet the boot switches to SCIF download mode, then press RESET "
