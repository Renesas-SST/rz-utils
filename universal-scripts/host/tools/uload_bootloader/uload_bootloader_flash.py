...
class UloadFlashUtil:
	...
	def __write_v4h_uload_bootloader(self):
		...
		try:
			xspi_region_start = int(self.__uloadFlashInfo['xspi_region_start'], 16)
			xspi_region_size = int(self.__uloadFlashInfo['xspi_region_size'], 16)
			if xspi_region_start < 0 or xspi_region_size <= 0:
				raise ValueError('xSPI region start/size must be non-negative')
			xspi_region_end = xspi_region_start + xspi_region_size
			max_sizes = {}
			configured_spans = []
			for label, _, _, offset_key in artifacts:
				offset = int(layout[offset_key], 16)
				size_key = 'bid_size' if offset_key == 'BID' else f'{offset_key.lower()}_max_size'
				max_size = int(layout[size_key], 16)
				if max_size <= 0:
					raise ValueError(f'{size_key} must be greater than zero')
				max_sizes[offset_key] = max_size
				configured_spans.append((label, offset, offset + max_size))
		except (KeyError, TypeError, ValueError) as exc:
			die(msg=f'Invalid V4H xSPI layout configuration: {exc}')

		configured_spans.sort(key=lambda item: item[1])
		for index, (label, start, end) in enumerate(configured_spans):
			if start < xspi_region_start or end > xspi_region_end:
				die(msg=(f'V4H xSPI layout overflow: {label} [0x{start:X}, 0x{end:X}) '
						 f'is outside [0x{xspi_region_start:X}, 0x{xspi_region_end:X})'))
			if index + 1 < len(configured_spans):
				next_label, next_start, _ = configured_spans[index + 1]
				if end > next_start:
					die(msg=(f'V4H xSPI layout overlap: {label} ends at 0x{end:X}, '
							 f'after {next_label} starts at 0x{next_start:X}'))

		print("V4H image sources on the SD card's FAT32 partition 1:")
		for label, path, _, _ in artifacts:
			print(f'  {label}: {path}')

		print('\nSelect normal boot mode, then press RESET (or power-cycle the board).')
		print('If the board is already at a U-Boot prompt, do not reset it.')
		...
		self.__writeSerialCmd(f'sf erase {xspi_region_start:X} {xspi_region_size:X}')
		response = self.__serialRead('=>')
		self.__v4h_require(response, 'Erased: OK', 'xSPI erase')

		for label, path, _, offset_key in artifacts:
			self.__write_v4h_xspi_file(
				load_address, verify_address, path, layout[offset_key], label,
				max_sizes[offset_key])
