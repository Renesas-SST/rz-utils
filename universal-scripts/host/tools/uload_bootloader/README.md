...
The tool accepts an existing U-Boot prompt. Otherwise, select normal boot mode
and press the Sparrow-Hawk **RESET** button; a power-cycle is an alternative.
...
> **RECOVERY SAFETY:** Before `sf erase`, all four payloads are loaded in turn
> and size-checked. They are loaded again individually for writing after the
> erase. If an error occurs after `Erasing V4H xSPI region...`, do not reset or
> power off the board: SPL/FIT may be erased or only partly written. Keep the
> current U-Boot prompt and SD card in place, then run the same Sparrow-Hawk
> command again. Do not remove or update the SD card during ULoad; prepare and
> verify all artifacts on the host before starting. Use the RESET button before
> `sf erase` or after successful completion, never while retrying from the
> current post-erase U-Boot prompt. If that prompt is lost, select SCIF download
> mode and recover with BootloaderFlash instead of attempting a normal xSPI
> boot.
...
**4. Power on or reset the board. It will load bootloader images from U-Boot into xSPI flash**
...
prompt. Do not reset it or remove the SD card. Rerun the same Sparrow-Hawk
command with the prepared SD card still inserted. Restart with RESET or a
power-cycle only after all four `Verified ... CRC32` messages and
`V4H ULoad bootloader flashing completed successfully` are printed. Do not use
the RESET button during this recovery interval; resetting also discards the
U-Boot instance currently running from RAM. If the prompt is no longer
available, select SCIF download mode and use BootloaderFlash to restore the xSPI
loader.
...
- Press RESET on boards that provide it; otherwise try power cycling the board
